...
if (isset($_POST['form_sent']) && $action == 'in')
{
	...
	$result = $db->query('SELECT id, group_id, password, save_pass, is_team FROM '.$db->prefix.'users WHERE '.$username_sql) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
	list($user_id, $group_id, $db_password_hash, $save_pass, $is_team) = $db->fetch_row($result);
	if (defined('PUN_SAVEPASS_OPTION'))
		$save_pass = (!empty($_POST['remember']))? '1': '0';

	$authorized = false;
	if (!empty($db_password_hash) && $is_team != '1')
	{
	...
	if (!$authorized)
		message($lang_login['Wrong user/pass'].' <a href="'.$base_url.'/login.php?action=forget">'.$lang_login['Forgotten pass'].'</a>');
	...
	redirect(htmlspecialchars($_POST['redirect_url']).((substr_count($_POST['redirect_url'],'?') == 1) ? '&' : '?').'login=1', $lang_login['Login redirect']);
}
...
{
	if ($pun_user['is_guest'] || !isset($_GET['id']) || $_GET['id'] != $pun_user['id'] || !isset($_GET['csrf_hash']) || $_GET['csrf_hash'] != pun_hash($pun_user['id'].pun_hash(get_remote_address())))
	{
...
//	set_tracked_topics(null);
	...
	redirect($base_url.'/index.php', $lang_login['Logout redirect']);
}
	...
	if (isset($_POST['form_sent']))
	{
		...
		if ($db->num_rows($result))
		{
			...
			while ($cur_hit = $db->fetch_assoc($result))
			{
				...
				$cur_mail_message = str_replace('<activation_url>', $base_url.'/profile.php?id='.$cur_hit['id'].'&action=change_pass&key='.$new_password_key, $cur_mail_message);
...
<?php if (defined('PUN_SAVEPASS_OPTION')): ?>							<br class="clearb" />
							<label class="conl"><input type="checkbox" name="remember" value="1" tabindex="3" />&nbsp;<strong><?php echo $lang_common['Remember login'] ?></strong><br /></label>
<?php endif; ?>
							<p class="clearb"><?php echo $lang_login['Login info'] ?></p>
							<p><a href="register.php" tabindex="5"><?php echo $lang_login['Not registered'] ?></a>&nbsp;&nbsp;
							<a href="login.php?action=forget" tabindex="6"><?php echo $lang_login['Forgotten pass'] ?></a></p>
						</div>
				</fieldset>
			</div>
			<p><input type="submit" name="login" value="<?php echo $lang_common['Login'] ?>" tabindex="4" /></p>
		</form>
	</div>
</div>
<?php
