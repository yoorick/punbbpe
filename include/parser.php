...
include PUN_ROOT.'include/smilies.php';
...
//	return '<a href="'.$full_url.'">'.$link.'</a>';
	if (preg_match('#^'.preg_quote($base_url, '#').'#i', str_replace('www.', '', $full_url)))
		return '<a class="internal" href="'.$full_url.'">'.$link.'</a>';
	else
		return '<a class="external" href="'.$full_url.'">'.$link.'</a>';
}


//
// Include Flash media player
//
function handle_media_tag($url, $link = '')
{
	global $pun_user, $base_url;
...
//	$link = ($link == '' || $link == $url) ? ((strlen($url) > 55) ? substr($url, 0 , 39).' &hellip; '.substr($url, -10) : $url) : stripslashes($link);
	...
	return '</p><p class="bbvideo"><object width="464" height="380" data="'.$base_url.'/players/mediaplayer.swf" classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=8,0,0,0"><param name="movie" value="'.$base_url.'/players/mediaplayer.swf" /><param name="menu" value="false" /><param name="quality" value="best" /><param name="allowFullScreen" value="true" /><param name="flashvars" value="frontcolor=0xEEEEEE&backcolor=0x222222&lightcolor=0xFFFFFF&autostart=false&usefullscreen=true&file='.$full_url.'" /><embed src="'.$base_url.'/players/mediaplayer.swf" width="464" height="380" menu="false" allowfullscreen="true" flashvars="frontcolor=0xEEEEEE&backcolor=0x222222&lightcolor=0xFFFFFF&autostart=false&usefullscreen=true&file='.$full_url.'" type="application/x-shockwave-flash" pluginspage= "http://www.macromedia.com/go/getflashplayer"/></embed></object></p><p>';
}
...
function handle_search_tag($where = 'forum', $what = '')
{
	...
	$context = urlencode($what);
	$context = str_replace('%26quot%3B', '%22', $context);

	switch($where) {
	case 'forum':
		$full_url = 'search.php?action=search&amp;keywords='.$context;
		break;
	case 'punres':
		$full_url = 'http://punres.org/search.php?action=search&amp;keywords='.$context;
		break;
	case 'google':
		$full_url = 'http://www.google.com/search?q='.$context;
		break;
	case 'yandex':
		$full_url = 'http://www.yandex.ru/yandsearch?text='.$context.'&amp;rpt=rad';
		break;
	default:
		return '[ERROR SEARCH]';
	}
...
//
function handle_img_tag($url, $is_signature = false, $subclass='')
{
	...
	if ($is_signature && $pun_user['show_img_sig'] != '0')
		...
		$img_tag = '<img class="postimg'.$subclass.'" src="'.$url.'" alt="'.htmlspecialchars($url).'" />';
...
function do_bbcode($text)
{
	...
	$replace = array('<strong>$1</strong>',
					 ...
					 '</p><p class="bbcenter">$1</p><p>',
					 '</p><p class="bbright">$1</p><p>',
					 '</p><p class="bbleft">$1</p><p>',
...
{
	global $smiley_text, $smiley_img, $base_url, $pun_config;
    ...
    // ::thumb###:: tag
    $prefix = $base_url.'/'.$pun_config['file_thumb_path'];
    $suffix = '-'.$pun_config['file_thumb_width'].'x'.$pun_config['file_thumb_height'].'-cut.jpg';
    $text = preg_replace('#::thumb([0-9]+)::#', '<a href="'.$base_url.'/download.php?aid=$1"><img src="'.$prefix.'$1'.$suffix.'" alt="thumb$1" /></a>', $text);
...
function parse_message($text, $hide_smilies)
{
	...
	if ($pun_config['p_message_bbcode'] == '1' && strpos($text, '[') !== false && strpos($text, ']') !== false)
	{
		...
		if ($pun_config['p_message_img_tag'] == '1')
		{
			...
			$text = preg_replace('#\[imgl\]((ht|f)tps?://)([^\s<"]*?)\[/imgl\]#e', 'handle_img_tag(\'$1$3\',false,\' imgl\')', $text);
			$text = preg_replace('#\[imgr\]((ht|f)tps?://)([^\s<"]*?)\[/imgr\]#e', 'handle_img_tag(\'$1$3\',false,\' imgr\')', $text);
		}
	}
...
//
//  Get short version of message preserving markups.
//  parameters:
//    string  $message     [in, out] - text to cut
//    integer $max_length  [in]      - length to cut
//  return TRUE when was cut
//
function smartcut(&$message, $max_length=SHORT_MSG_LEN)
{
	// optimization: if ALL text is too short
	if (!isset($message) || (mb_strlen($message) <= $max_length))
		return false;

	$pattern = '#((?:\[|\[/)(?:b|i|u|s|url|url=(?:[^\[]*?)|email|email=(?:[^\[]*?)|mono|size=(?:[0-9]{2})|center|right|quote|quote=(?:&quot;|"|\'|)(?:.*)|img|imgr|imgl|code|color|color=(?:[a-zA-Z]*|\#?[0-9a-fA-F]{3,6})|listo|list|li)\])#u';
	$result = preg_split($pattern, $message, -1, PREG_SPLIT_DELIM_CAPTURE );

	// optimization: if HAVE NOT any tag
	if (count($result) == 1)
	{
	...
	for ($i=0; $i < count($result); $i++)
	{
		...
		if ($trunc_at == -1)
		{
			...
			{
				if ($length + mb_strlen($clause) >= $max_length)
				{
					// truncate text
					$result[$i] = mb_substr($clause, 0, $max_length-$length);
					$trunc_at = $i;
				}
				$length += mb_strlen($clause);
			}
			else
			{
		...
		else
		{
			...
			else
			{
				...
				else if (!$was_code && preg_match('#\[([a-z]+)#', $clause, $regs) !== false)
				{
					$result[$i] = '';

					// find & destroy the respective closing tag
					$clause = '[/'.$regs[1].']';
	...
	// optimization: if result HAVE NOT truncated, do not touch message
	if ($trunc_at == -1)
		return false;
	else
	{
