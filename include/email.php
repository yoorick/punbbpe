...
function pun_mail($to, $subject, $message, $from = '')
{
	...
	$encoding = (isset($lang_common['mail_encoding']))? $lang_common['mail_encoding'] : $lang_common['lang_encoding'];
	$sender = str_replace('"', '', $pun_config['o_board_title'].' '.$lang_common['Mailer']);
	if (strcasecmp($encoding, $lang_common['lang_encoding']) != 0)
	{
		$subject = iconv($lang_common['lang_encoding'], $encoding, $subject);
		$message = iconv($lang_common['lang_encoding'], $encoding, $message);
		$sender  = iconv($lang_common['lang_encoding'], $encoding, $sender);
	}
