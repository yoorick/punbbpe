...
//
function generate_quickjump_cache($group_id = false, $kind = null)
{
	global $db, $lang_common, $pun_user, $kinds;
	...
	// If a kind of board was supplied, we generate the quickjump cache for that kind only
	if ($kind !== null)
		$kind_ids[] = $kind;
	else
		$kind_ids = array_keys($kinds);

	foreach	($kind_ids as $kind)
	{
		...
		while (list(, $group_id) = @each($groups))
		{
			...
			$fh = @fopen(PUN_ROOT.'cache/cache_quickjump_'.$group_id.'_'.$kind.'.php', 'wb');
			if (!$fh)
			...
			$output .= "\t\t\t\t".'<form id="qjump" method="get" action="viewboard.php">'."\n\t\t\t\t\t".'<div><label><?php echo $lang_common[\'Jump to\'] ?>'."\n\n\t\t\t\t\t".'<br /><select name="id" onchange="window.location=(\'viewboard.php?id=\'+this.options[this.selectedIndex].value)">'."\n";


			$result = $db->query('SELECT '.
					'c.id AS cid, c.cat_name, '.
					'f.id AS fid, f.forum_name, f.redirect_url '.
				'FROM '.$db->prefix.'categories AS c INNER JOIN '.
					$db->prefix.'forums AS f ON c.id=f.cat_id LEFT JOIN '.
					$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$group_id.') '.
				'WHERE '.
					'(fp.read_forum IS NULL OR fp.read_forum=1) AND '.
					'(c.kind='.$kind.') '.
				'ORDER BY c.disp_position, c.id, f.disp_position', false)
			...
			while ($cur_board = $db->fetch_assoc($result))
			{
				if ($cur_board['cid'] != $cur_category)	// A new category since last iteration?
				{
					...
					$output .= "\t\t\t\t\t\t".'<optgroup label="'.pun_htmlspecialchars($cur_board['cat_name']).'">'."\n";
					$cur_category = $cur_board['cid'];
				}

				$redirect_tag = ($cur_board['redirect_url'] != '') ? ' &gt;&gt;&gt;' : '';
				$output .= "\t\t\t\t\t\t\t".'<option value="'.$cur_board['fid'].'"<?php echo ($board_id == '.$cur_board['fid'].') ? \' selected="selected"\' : \'\' ?>>'.pun_htmlspecialchars($cur_board['forum_name']).$redirect_tag.'</option>'."\n";
			}
