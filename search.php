...
if (!$pun_user['is_guest'])
{
	...
	$context_menu[] = '<a href="'.$base_url.'/search.php?action=show_label">'.$lang_common['Labels cloud'].'</a>';
}
...
if (isset($_GET['action']) || isset($_GET['search_id']))
{
	...
	$cat_kinds = (isset($_GET['cat_kinds']) && count($_GET['cat_kinds']) > 0) ? array_values($_GET['cat_kinds']) : null;
	$kind = (isset($_GET['kind'])) ? intval($_GET['kind']) : null;
...
//			message($lang_common['Bad request']);
		{
			...
			$page_title = $lang_common['Labels cloud'];
			include PUN_ROOT.'include/header.php';
			include PUN_ROOT.'include/user/cloud.php';
			include PUN_ROOT.'include/footer.php';
		}
		$label = mb_strtolower(trim($_GET['text']));
	}
	else
	{
	...
	if (isset($search_id))
	{
		...
		if ($row = $db->fetch_assoc($result))
		{
			...
			$descr = $temp['descr'];
			if (isset($temp['kind'])&&intval(intval($temp['kind']))>=0)
				$kind = intval($temp['kind']);
	...
	else
	{
		...
		// Search a specific forum(s)?
		if ($forum != -1)
		{
			$cat_sql = '';
			$forum_sql = ' AND t.forum_id = '.$forum;
		}
		else if (!empty($cat_kinds))
		{
			$cat_sql = ' INNER JOIN '.$db->prefix.'categories AS c ON f.cat_id=c.id ';
			$forum_sql = ' AND c.kind IN ('.implode(',',$cat_kinds).')';
		}
		else
		{
			$cat_sql = '';
		...
		{
			$descr = '';
			if (!empty($author)) $descr .= $lang_common['Author'].': '.$author.'. ';
			if (!empty($keywords)) $descr .= '"'.$keywords.'"';
			...
			if ($show_as == 'topics')
			{
				$result = $db->query('SELECT t.id FROM '.$db->prefix.'posts AS p INNER JOIN '.$db->prefix.'topics AS t ON t.id=p.topic_id INNER JOIN '.$db->prefix.'forums AS f ON f.id=t.forum_id '.$cat_sql.'LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND p.id IN('.implode(',', $search_ids).')'.$forum_sql.' GROUP BY t.id', true) or error('Unable to fetch topic list', __FILE__, __LINE__, $db->error());
			...
			{
				$result = $db->query('SELECT p.id FROM '.$db->prefix.'posts AS p INNER JOIN '.$db->prefix.'topics AS t ON t.id=p.topic_id INNER JOIN '.$db->prefix.'forums AS f ON f.id=t.forum_id '.$cat_sql.'LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND p.id IN('.implode(',', $search_ids).')'.$forum_sql, true) or error('Unable to fetch topic list', __FILE__, __LINE__, $db->error());
		...
		else if ($action == 'show_new' || $action == 'show_24h' || $action == 'show_user' || $action == 'show_label' || $action == 'show_subscriptions' || $action == 'show_unanswered')
		{
			if (isset($kind))
			{
				$category_sql = ' INNER JOIN '.$db->prefix.'categories AS c ON (c.id=f.cat_id) AND (c.kind='.$kind.') ';
				$pre_descr = $lang_common['Boards kind'][$kind].': ';
			}
			else
			{
				$category_sql = '';
			...
			if ($action == 'show_new')
			{
				$descr = $pre_descr.$lang_common['Show new posts'];
				if ($pun_user['is_guest'])
				...
				if (!$num_hits)
					message($pre_descr.$lang_search['No new posts']);
			}
			// If it's a search for todays posts
			else if ($action == 'show_24h')
			{
				$descr = $pre_descr.$lang_common['Show recent posts'];
				$result = $db->query('SELECT t.id FROM '.
					$db->prefix.'topics AS t INNER JOIN '.
					$db->prefix.'forums AS f ON f.id=t.forum_id LEFT JOIN '.
					$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') '.
					$category_sql.
					'WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND t.last_post>'.(time() - 86400).' AND t.moved_to IS NULL') 
					or error('Unable to fetch topic list', __FILE__, __LINE__, $db->error());
				$num_hits = $db->num_rows($result);

				if (!$num_hits)
					message($pre_descr.$lang_search['No recent posts']);
			}
			...
			else if ($action == 'show_user')
			{
				...
				$username = pun_htmlspecialchars($db->result($result));
				$descr = $pre_descr.sprintf($lang_search['Topics with post of'], $username);
				$result = $db->query('SELECT t.id FROM '.
					$db->prefix.'topics AS t INNER JOIN '.
					$db->prefix.'posts AS p ON t.id=p.topic_id INNER JOIN '.
					$db->prefix.'forums AS f ON f.id=t.forum_id LEFT JOIN '.
					$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') '.
					$category_sql.
					'WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND p.poster_id='.$user_id.' GROUP BY t.id') 
					or error('Unable to fetch topic list', __FILE__, __LINE__, $db->error());
				$num_hits = $db->num_rows($result);

				if (!$num_hits)
					message($pre_descr.$lang_search['No user posts']);
			}
			// If it's a search for topics marked by some label
			else if ($action == 'show_label')
			{
				if (empty($pun_config['o_topic_labels']))
					message($lang_search['No labels']);

				$cond = $db->escape($label);
				$descr = $pre_descr.sprintf($lang_search['Topics with label'], $label);
				$result = $db->query('SELECT t.id FROM '.$db->prefix.'topics AS t INNER JOIN '.$db->prefix.'forums AS f ON f.id=t.forum_id LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND t.labels like \'%,'.$cond.',%\'') or error('Unable to fetch topic list', __FILE__, __LINE__, $db->error());
				$num_hits = $db->num_rows($result);
			...
			{
				$descr = $pre_descr.$lang_common['Show subscriptions'];
				if ($pun_user['is_guest'])
					message($lang_common['Bad request']);

				$result = $db->query('SELECT t.id FROM '.
					$db->prefix.'topics AS t INNER JOIN '.
					$db->prefix.'subscriptions AS s ON (t.id=s.topic_id AND s.user_id='.$pun_user['id'].') INNER JOIN '.
					$db->prefix.'forums AS f ON f.id=t.forum_id LEFT JOIN '.
					$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') '.
					$category_sql.
				...
				if (!$num_hits)
					message($pre_descr.$lang_search['No subscriptions']);
			}
			// If it's a search for unanswered posts
			else
			{
				$descr = $pre_descr.$lang_common['Show unanswered posts'];
				$result = $db->query('SELECT t.id FROM '.
					$db->prefix.'topics AS t INNER JOIN '.
					$db->prefix.'forums AS f ON f.id=t.forum_id LEFT JOIN '.
					$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') '.
				...
				if (!$num_hits)
					message($pre_descr.$lang_search['No unanswered']);
			}
		...
		if ($action != 'show_new' && $action != 'show_24h' && $action != 'show_label')
		{
			$db->end_transaction();
			$db->close();

			// Redirect the user to the cached result page
			header('Location: '.$base_url.'/search.php?search_id='.$search_id);
			exit;
		}
	}
	...
	if ($search_results != '')
	{
		...
		if ($show_as == 'posts')
		{
			...
			$sql = 'SELECT t.id AS tid, t.poster, t.subject, t.topic_desc, t.last_post, t.last_post_id, t.last_poster, t.num_replies, t.closed, t.forum_id, t.labels FROM '.$db->prefix.'topics AS t WHERE t.id IN('.$search_results.') ORDER BY '.$sort_by_sql;
		...
		$paging_links = $lang_common['Pages'].': '.paginate($num_pages, $p, $base_url.'/search.php?search_id='.$search_id);
		$post_link = '';
		$bread_crumbs = '<ul><li><a href="'.$base_url.'/search.php">'.$lang_common['Search'].'</a>&nbsp;</li><li>&raquo;&nbsp;</li><li>'.pun_htmlspecialchars($descr).'</li></ul>'; // [!!!] temporary
		...
		<div class="inbox">
			...
			<thead>
				<tr>
					...
					<th class="tc2" scope="col"><?php echo $lang_common['Board'] ?></th>
...
<?php
		...
		$result = $db->query('SELECT f.id, f.forum_name, c.kind FROM '.$db->prefix.'forums AS f INNER JOIN '.$db->prefix.'categories AS c ON f.cat_id=c.id') or error('Unable to fetch forum list', __FILE__, __LINE__, $db->error());
		...
		for ($i = 0; $i < count($search_set); ++$i)
		{
			...
			while (list(, $temp) = @each($forum_list))
			{
				if ($temp[0] == $search_set[$i]['forum_id'])
					$forum = $lang_common['Board kind'][$temp[2]].' <a href="'.$base_url.'/viewboard.php?id='.$temp[0].'">'.pun_htmlspecialchars($temp[1]).'</a>';
			}
			...
			if ($show_as == 'posts')
			{
				$topic_url = $base_url.'/viewtopic.php?id='.$search_set[$i]['tid'];
				$post_url = $base_url.'/viewtopic.php?pid='.$search_set[$i]['pid'].'#p'.$search_set[$i]['pid'];

				$icon = '<div class="icon"><div class="nosize">'.$lang_common['Normal icon'].'</div></div>';
				$subject = '<a href="'.$topic_url.'">'.pun_htmlspecialchars($search_set[$i]['subject']).'</a>';
				...
				$was_cut = smartcut($message);
				$message = parse_message($message, !empty($search_set[$i]['hide_smilies'])).($was_cut ? ('<a href="'.$post_url.'">'.$lang_common['continue reading'].'</a>') : '');
...
<div class="blockpost searchposts<?php echo $vtbg ?>">
	<h2><?php echo $forum ?>&nbsp;&raquo;&nbsp;<?php echo $subject ?>&nbsp;&raquo;&nbsp;<a href="<?php echo $post_url ?>"><?php echo format_time($search_set[$i]['pposted']) ?></a></h2>
	<div class="box">
		<div class="inbox">
			<div class="postleft">
				<dl>
					...
					<dd><?php echo $icon; ?></dd>
					<dd><p class="clearb"><a href="<?php echo $post_url ?>"><?php echo $lang_search['Go to post'] ?></a></p></dd>
				</dl>
			</div>
			<div class="postright">
				<div class="postmsg">
...
<?php
			...
			else
			{
				...
				$subject = '<a href="'.$topic_url.'">'.pun_htmlspecialchars($search_set[$i]['subject']).'</a> <span class="byuser">'.$lang_common['by'].'&nbsp;'.pun_htmlspecialchars($search_set[$i]['poster']).'</span>';
				if (trim($search_set[$i]['topic_desc']) != '') 
					$topic_desc = parse_message($search_set[$i]['topic_desc'], false); 
				else
					$topic_desc = '';
				...
				if (!$pun_user['is_guest'] && $search_set[$i]['last_post'] > $pun_user['last_visit'])
				{
					...
					$subject_new_posts = '<span class="newtext">[&nbsp;<a href="'.$topic_url.'&amp;action=new" title="'.$lang_common['New posts info'].'">'.$lang_common['New posts'].'</a>&nbsp;]</span>';
				}
				else
				...
				if ($num_pages_topic > 1)
					$subject_multipage = '[ '.paginate($num_pages_topic, -1, $base_url.'/viewtopic.php?id='.$search_set[$i]['tid']).' ]';
				else
				...
				$labels = (!empty($pun_config['o_topic_labels']))? $search_set[$i]['labels']: '';
...
?>
				<tr<?php if ($item_status != '') echo ' class="'.trim($item_status).'"'; ?>>
					...
					<td class="tcr"><?php echo '<a href="'.$base_url.'/viewtopic.php?pid='.$search_set[$i]['last_post_id'].'#p'.$search_set[$i]['last_post_id'].'">'.format_time($search_set[$i]['last_post']).'</a> '.$lang_common['by'].'&nbsp;'.pun_htmlspecialchars($search_set[$i]['last_poster']) ?></td>
				</tr>
<?php
...
{
?>
						<label class="conl"><?php echo $lang_search['Search in kinds'] ?></label>
						<br class="clearb"/>
<?php
	foreach (array_keys($kinds) as $kind)
	{
		$checkboxes[] = '<label style="DISPLAY: inline"><input type="checkbox" name="cat_kinds[]" value="'.$kind.'" checked="checked" />&nbsp;'.$lang_common['Boards kind'][$kind].'</label>';
	}
	echo "\t\t\t\t\t\t\t".implode('&nbsp; ',$checkboxes)."\n";
	echo "\t\t\t\t\t\t\t".'<br class="clearb" />'."\n";
}
?>
						<label class="conl"><?php echo (($pun_config['o_search_all_forums'] == '1' || $pun_user['g_id'] < PUN_GUEST)? $lang_search['or'] : '') . $lang_search['Board search'] ?>
						<br /><select id="forum" name="forum">
<?php

if ($pun_config['o_search_all_forums'] == '1' || $pun_user['g_id'] < PUN_GUEST)
	echo "\t\t\t\t\t\t\t".'<option value="-1">'.$lang_search['All boards'].'</option>'."\n";

$result = $db->query('SELECT c.id AS cid, c.cat_name, c.kind, f.id AS fid, f.forum_name, f.redirect_url FROM '.$db->prefix.'categories AS c INNER JOIN '.$db->prefix.'forums AS f ON c.id=f.cat_id LEFT JOIN '.$db->prefix.'forum_perms AS fp ON (fp.forum_id=f.id AND fp.group_id='.$pun_user['g_id'].') WHERE (fp.read_forum IS NULL OR fp.read_forum=1) AND f.redirect_url IS NULL ORDER BY c.disp_position, c.id, f.disp_position', true) or error('Unable to fetch category/forum list', __FILE__, __LINE__, $db->error());
...
while ($cur_board = $db->fetch_assoc($result))
{
	if ($cur_board['cid'] != $cur_category)	// A new category since last iteration?
	{
		...
		echo "\t\t\t\t\t\t\t".'<optgroup label="'.pun_htmlspecialchars($cur_board['cat_name']).' ('.$lang_common['Boards kind'][$cur_board['kind']].')">'."\n";
		$cur_category = $cur_board['cid'];
	}

	echo "\t\t\t\t\t\t\t\t".'<option value="'.$cur_board['fid'].'">'.pun_htmlspecialchars($cur_board['forum_name']).'</option>'."\n";
}
				...
				<fieldset>
					...
					<div class="infldset">
						...
						<br /><select name="sort_by">
							...
							<option value="3"><?php echo $lang_search['Sort by board'] ?></option>
						</select>
						<br /></label>
