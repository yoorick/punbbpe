...
?>
	<div class="blockform">
		...
		<div class="box">
			<form id="groups2" method="post" action="<?php echo $base_url ?>/admin/groups.php" onsubmit="return process_form(this)">
...
<?php endif; ?>								<tr>
								...
								<tr>
									...
									<td>
										<input type="radio" name="file_download" value="1"<?php if ($group['g_file_download'] == '1') echo ' checked="checked"' ?> tabindex="22" />&nbsp;<strong>Yes</strong>&nbsp;&nbsp;&nbsp;<input type="radio" name="file_download" value="0"<?php if ($group['g_file_download'] == '0') echo ' checked="checked"' ?> tabindex="24" />&nbsp;<strong>No</strong>
										<span>Allow users in this group to download files from attachments.</span>
									</td>
								</tr>
<?php if ($group['g_id'] != PUN_GUEST): ?>								<tr>
									<th scope="row">Upload files</th>
									<td>
										<input type="radio" name="file_upload" value="1"<?php if ($group['g_file_upload'] == '1') echo ' checked="checked"' ?> tabindex="23" />&nbsp;<strong>Yes</strong>&nbsp;&nbsp;&nbsp;<input type="radio" name="file_upload" value="0"<?php if ($group['g_file_upload'] == '0') echo ' checked="checked"' ?> tabindex="26" />&nbsp;<strong>No</strong>
										<span>Allow users in this group to attach files in posts.</span>
									</td>
								</tr>
								<tr>
									<th scope="row">Uload files limit</th>
									<td>
										<input type="text" name="file_limit" size="5" maxlength="4" value="<?php echo $group['g_file_limit'] ?>" tabindex="24" />
										<span>Number of files that users in this group may to upload. Set to 0 to disable.</span>
									</td>
								</tr>
								<tr>
...
{
	confirm_referrer('admin/groups.php');
	...
	if ($_POST['mode'] == 'add')
	{
		...
		while ($cur_board_perm = $db->fetch_assoc($result))
			$db->query('INSERT INTO '.$db->prefix.'forum_perms (group_id, forum_id, read_forum, post_replies, post_topics) VALUES('.$new_group_id.', '.$cur_board_perm['forum_id'].', '.$cur_board_perm['read_forum'].', '.$cur_board_perm['post_replies'].', '.$cur_board_perm['post_topics'].')') or error('Unable to insert group forum permissions', __FILE__, __LINE__, $db->error());
	}
	else
	{
	...
	redirect($base_url.'/admin/groups.php', 'Group '.(($_POST['mode'] == 'edit') ? 'edited' : 'added').'. Redirecting &hellip;');
}
...
else if (isset($_POST['set_default_group']))
{
	...
	redirect($base_url.'/admin/groups.php', 'Default group set. Redirecting &hellip;');
}
...
{
	confirm_referrer('admin/groups.php', true);
	...
	if (!$db->num_rows($result) || isset($_POST['del_group']))
	{
		...
		redirect($base_url.'/admin/groups.php', 'Group removed. Redirecting &hellip;');
	}
...
?>
	<div class="blockform">
		...
		<div class="box">
			<form id="groups" method="post" action="<?php echo $base_url ?>/admin/groups.php?del_group=<?php echo $group_id ?>">
				<div class="inform">
					<input type="hidden" name="csrf_hash" value="<?php echo csrf_hash() ?>" />
					<fieldset>
...
?>
	<div class="blockform">
		...
		<div class="box">
			<form id="groups" method="post" action="<?php echo $base_url ?>/admin/groups.php?action=foo">
				<div class="inform">
					<input type="hidden" name="csrf_hash" value="<?php echo csrf_hash() ?>" />
					<fieldset>
...
while ($cur_group = $db->fetch_assoc($result))
	echo "\t\t\t\t\t\t\t\t".'<tr><th scope="row"><a href="'.$base_url.'/admin/groups.php?edit_group='.$cur_group['g_id'].'">Edit</a>'.(($cur_group['g_id'] > PUN_MEMBER) ? ' - <a href="'.$base_url.'/admin/groups.php?del_group='.$cur_group['g_id'].'&amp;csrf_hash='.csrf_hash().'">Remove</a>' : '').'</th><td>'.pun_htmlspecialchars($cur_group['g_title']).'</td></tr>'."\n";
