...
if (isset($_POST['form_sent']))
{
	...
	if (!preg_match('#^'.preg_quote(str_replace('www.', '', $base_url).'/admin/options.php', '#').'#i', str_replace('www.', '', (isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : ''))))
		message('Bad HTTP_REFERER. If you have moved these forums from one location to another or switched domains, you need to update the Base URL manually in the database (look for base_url in the config.php file) and then clear the cache by deleting all .php files in the /cache directory.');
	...
	$form['photos_dir'] = str_replace("\0", '', $form['photos_dir']);
	$form['avatars_dir'] = str_replace("\0", '', $form['avatars_dir']);
	$form['badges_dir'] = str_replace("\0", '', $form['badges_dir']);

	// Make sure directories doesn't end with a slash
	if (substr($form['photos_dir'], -1) == '/')
		$form['photos_dir'] = substr($form['photos_dir'], 0, -1);
	if (substr($form['avatars_dir'], -1) == '/')
		$form['avatars_dir'] = substr($form['avatars_dir'], 0, -1);
	if (substr($form['badges_dir'], -1) == '/')
		$form['badges_dir'] = substr($form['badges_dir'], 0, -1);
	...
	$form['photos_width'] = intval($form['photos_width']);
	$form['photos_height'] = intval($form['photos_height']);
	$form['avatars_width'] = intval($form['avatars_width']);
	$form['avatars_height'] = intval($form['avatars_height']);
	$form['badges_width'] = intval($form['badges_width']);
	$form['badges_height'] = intval($form['badges_height']);
	...
	redirect($base_url.'/admin/options.php', 'Options updated. Redirecting &hellip;');
}
...
?>
	<div class="blockform">
		...
		<div class="box">
			<form method="post" action="<?php echo $base_url ?>/admin/options.php?action=foo">
						...
						<div class="infldset">
							<table class="aligntop" cellspacing="0">
								...
								<!-- <tr>
									...
									<td>
										<input type="text" name="form[base_url]" size="50" maxlength="100" value="<?php echo $base_url ?>" />
							...
							<table class="aligntop" cellspacing="0">
								...
								<tr>
									...
									<td>
										...
										<span>Enable this to use user ranks. See <a href="<?php echo $base_url ?>/admin/ranks.php">Ranks</a> for more info.</span>
									</td>
								</tr>
								<tr>
									...
									<td>
										...
										<span>This feature displays a dot in front of topics in viewboard.php in case the currently logged in user has posted in that topic earlier. Disable if you are experiencing high server load.</span>
									</td>
								</tr>
								<tr>
								...
								<tr>
									<th scope="row">Use Labels for topic</th>
									<td>
										<input type="radio" name="form[topic_labels]" value="1"<?php if ($pun_config['o_topic_labels'] == '1') echo ' checked="checked"' ?> />&nbsp;<strong>Yes</strong>&nbsp;&nbsp;&nbsp;<input type="radio" name="form[topic_labels]" value="0"<?php if ($pun_config['o_topic_labels'] == '0') echo ' checked="checked"' ?> />&nbsp;<strong>No</strong>
										<span>When enable, authors can mark their topics by comma-separated list of labels. Anybody can quick search all topics with the same label.</span>
									</td>
								</tr>
								<tr>
							...
							<table class="aligntop" cellspacing="0">
								<tr>
									...
									<td>
										...
										<span>When enabled, users will be able to upload an user avatar and team badge which will be displayed under their title/rank.</span>
									</td>
								</tr>
								<tr>
									<th scope="row">Photo upload directory</th>
									<td>
										<input type="text" name="form[photos_dir]" size="35" maxlength="50" value="<?php echo pun_htmlspecialchars($pun_config['o_photos_dir']) ?>" />
										<span>The upload directory for user photos (relative to the PunBB root directory). PHP must have write permissions to this directory.</span>
									</td>
								</tr>
								<tr>
									<th scope="row">Avatar upload directory</th>
									<td>
								...
								<tr>
									<th scope="row">Badge upload directory</th>
									<td>
										<input type="text" name="form[badges_dir]" size="35" maxlength="50" value="<?php echo pun_htmlspecialchars($pun_config['o_badges_dir']) ?>" />
										<span>The upload directory for badges (relative to the PunBB root directory). PHP must have write permissions to this directory.</span>
									</td>
								</tr>
								<tr>
									<th scope="row">Photo dimension</th>
									<td>
										<input type="text" name="form[photos_width]" size="5" maxlength="5" value="<?php echo $pun_config['o_photos_width'] ?>" /> x
										<input type="text" name="form[photos_height]" size="5" maxlength="5" value="<?php echo $pun_config['o_photos_height'] ?>" />
										<span>The maximum width and height of avatars in pixels (60x60 is recommended).</span>
									</td>
								</tr>
								<tr>
									<th scope="row">Avatar dimension</th>
									<td>
										<input type="text" name="form[avatars_width]" size="5" maxlength="5" value="<?php echo $pun_config['o_avatars_width'] ?>" /> x
										<input type="text" name="form[avatars_height]" size="5" maxlength="5" value="<?php echo $pun_config['o_avatars_height'] ?>" />
										<span>The maximum width and height of avatars in pixels (60x60 is recommended).</span>
									</td>
								</tr>
								<tr>
									<th scope="row">Badge dimension</th>
									<td>
										<input type="text" name="form[badges_width]" size="5" maxlength="5" value="<?php echo $pun_config['o_badges_width'] ?>" /> x
										<input type="text" name="form[badges_height]" size="5" maxlength="5" value="<?php echo $pun_config['o_badges_height'] ?>" />
										<span>The maximum width and height of badges in pixels (84x31 is recommended).</span>
									</td>
								</tr>
								<tr>
									...
									<td>
										...
										<span>The maximum allowed size of pictures in bytes (10240 is recommended).
										The maximum size of user photo sets in <a href="<?php echo $base_url.'/admin/files.php' ?>">File Options</a>.</span>
									</td>
								</tr>
							</table>
						</div>
