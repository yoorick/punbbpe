...
{
	confirm_referrer('admin/categories.php');

	$new_cat_name = trim($_POST['new_cat_name']);
	if ($new_cat_name == '')
		message('You must enter a name for the category.');

	$add_to_kind = intval($_POST['add_to_kind']);

	$db->query('INSERT INTO '.$db->prefix.'categories (cat_name, kind) VALUES(\''.$db->escape($new_cat_name).'\', '.$add_to_kind.')') or error('Unable to create category', __FILE__, __LINE__, $db->error());

	redirect($base_url.'/admin/categories.php', 'Category added. Redirecting &hellip;');
}
...
else if (isset($_POST['del_cat']) || isset($_POST['del_cat_comply']))
{
	...
	if (isset($_POST['del_cat_comply']))	// Delete a category with all boards and posts
	{
		@set_time_limit(0);
		...
		{
			$cur_board = $db->result($result, $i);

			// Prune all posts and topics
			prune($cur_board, 1, -1);

			// Delete the forum
			$db->query('DELETE FROM '.$db->prefix.'forums WHERE id='.$cur_board) or error('Unable to delete forum', __FILE__, __LINE__, $db->error());
		}
		...
		redirect($base_url.'/admin/categories.php', 'Category deleted. Redirecting &hellip;');
	}
				...
				<div class="inform">
					...
					<fieldset>
						...
						<div class="infldset">
							...
							<p>WARNING! Deleting a category will delete all boards and posts (if any) in that category!</p>
						</div>
					</fieldset>
				</div>
...
else if (isset($_POST['update']))	// Change position and name of the categories
{
	...
	redirect($base_url.'/admin/categories.php', 'Categories updated. Redirecting &hellip;');
}


// Generate an array with all categories
$result = $db->query('SELECT id, cat_name, disp_position, kind FROM '.$db->prefix.'categories ORDER BY disp_position') or error('Unable to fetch category list', __FILE__, __LINE__, $db->error());
$num_cats = $db->num_rows($result);
...
?>
	<div class="blockform">
		...
		<div class="box">
		<form method="post" action="<?php echo $base_url ?>/admin/categories.php?action=foo">
			<div class="inform">
				<input type="hidden" name="csrf_hash" value="<?php echo csrf_hash() ?>" />
				<fieldset>
					...
					<div class="infldset">
						<table class="aligntop" cellspacing="0">
							<tr>
								<th scope="row">Add a new category<div><input type="submit" name="add_cat" value="Add New" tabindex="3" /></div></th>
								<td>
									<input type="text" name="new_cat_name" size="35" maxlength="80" tabindex="1" />
									<select name="add_to_kind" tabindex="2">
<?php
foreach (array_keys($kinds) as $kind)
{
	echo "\t\t\t\t\t\t\t\t\t\t".'<option value="'.$kind.'">'.$lang_common['Boards kind'][$kind].'</option>'."\n";
}
?>
									</select>
									<span>The name and kind of the new category you want to add. You can edit the name of the category later (see below).
									You set the kind of new category only once. <br />Go to <a href="<?php echo $base_url ?>/admin/boards.php">Admin Boards</a> to add boards to your new category.</span>
								</td>
							</tr>
<?php if ($num_cats): ?>							<tr>
								<th scope="row">Delete a category<div><input type="submit" name="del_cat" value="Delete" tabindex="5" /></div></th>
								<td>
									<select name="cat_to_delete" tabindex="4">
<?php

	while (list(, list($cat_id, $cat_name, , $cat_kind)) = @each($cat_list))
		echo "\t\t\t\t\t\t\t\t\t\t".'<option value="'.$cat_id.'">'.pun_htmlspecialchars($cat_name).' ('.$lang_common['Boards kind'][$cat_kind].')</option>'."\n";
					...
					<div class="infldset">
						...
						<thead>
							<tr>
								...
								<th class="tc2">Kind</th>
								<th class="tcr" scope="col">Position</th>
								<th>&nbsp;</th>
							</tr>
						</thead>
						<tbody>
<?php
	...
	for ($i = 0; $i < $num_cats; ++$i)
	{
		list(, list($cat_id, $cat_name, $position, $cat_kind)) = @each($cat_list);

?>
							<tr style="vertical-align: middle">
								<td class="tcl"><input type="text" name="cat_name[<?php echo $i ?>]" value="<?php echo pun_htmlspecialchars($cat_name) ?>" size="35" maxlength="80" /></td>
								<td class="tc2"><?php echo $lang_common['Boards kind'][$cat_kind] ?></td>
								<td class="tcr"><input type="text" name="cat_order[<?php echo $i ?>]" value="<?php echo $position ?>" size="3" maxlength="3" /></td><td>&nbsp;</td>
							</tr>
<?php
